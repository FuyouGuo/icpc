#include "builtin.cpp"
#ifndef _GLIBCXX_CUSTOM_KUANGBIN_KMP
// next[i-next[i]...i-1] == next[0...next[i]-1]
// next[i-z...i-1] == next[0...z-1]
void kmp_next(char x[], int m, int next[]){
    int i,j;
    j=next[0]=-1;
    while(i<m){
        while(j!=-1&&x[i]!=x[j]) j=next[j];
        x[++i]=++j;
    }
}

// kmp_next[i]的意思是kmp_next[i]=next[next[...next[i]]](直到next[i]=-1或next[next'[i]]!=x[i])
void kmp_next_ext(char x[], int m, int kmp_next[]){
    int i,j;
    j=x[0]=-1;
    while(i<m){
        while(-1!=j&&x[i]!=x[j]) j=kmp_next[j];
        if(x[++i]==x[++j]) kmp_next[i]=kmp_next[j];
        else kmp_next[i]=j;
    }
}

// 返回x在y中出现的次数，可以重叠
int kmp_cnt(char x[], const int m, char y[], const int n, int next[]){
    int i,j;
    int ans;
    kmp_next(x,m,next);
    while(i<n){
        while(-1!=j&&y[i]!=x[j]) j=next[i];
        i++,j++;
        if(j>=m){
            ans++;
            j=next[j];
        }
    }
    return ans;
}

/*
 * 扩展kmp算法 (求出所有可匹配的串)
 * https://subetter.com/algorithm/extended-kmp-algorithm.html
 * next[i]:x[i...m-1] 与 x[0...m-1]的最长公共前缀
 * extend[i]:y[i...n-1] 与 x[0...m-1]的最长公共前缀
 */
void ext_kmp_next(char x[], int m, int next[]){
    next[0] = m;
    int j = 0;
    while(j+1<m&&x[j]==x[j+1]) j++;
    next[1] = j;
    int k = 1;
    for(int i = 2; i < m; i++){
        int p = next[k] + k - 1;   // 以k为起始位置的字符匹配成功的最右边界 x[0...p-k] = x[k...p]
        int l = next[i-k];
        if(i + l < p + 1) {
            next[i]=l;
        } else{
            j = max(0, p-i+1);  // j 已匹配长度, i >= p (举个例子，没有一个字符匹配的情况)
            while( i+j < m && x[i+j] == x[j])j++; next[i] = j;
            k = i;
        }
    }
}

void ext_kmp(char x[],int m,char y[],int n,int next[],int extend[]){
    ext_kmp_next(x,m,next);
    int j=0;
    while(j<n && j < m && x[j]==y[j+1])j++;
    extend[0]=j;
    int k = 0;
    for(int i = 1; i < n; i++){
        int p = extend[k] + k - 1;
        int l = next[i-k];
        if(i+l < p+1) extend[i]=l;
        else{
            j=max(0,p-i+1);
            while(i+j<n&&j<m&&y[i+j]==x[j])j++;
            extend[i]=j;
            k=i;
        }
    } 
}
#endif
