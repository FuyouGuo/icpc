...
namespace mySTL{
	...
	il void write(string s,int st=0){int len=s.length();if(st>=len)ret;for(int i=st;i<len;i++) pc(s[i]);}
...
namespace myDS{
	class DSU{public:vector<int>father_of_DSU;int size_of_DSU;
		il void Init(int n){size_of_DSU=n;father_of_DSU.clear();father_of_DSU.push_back(0);for(int i=1;i<=n;i++){father_of_DSU.push_back(i);}}
	...
	il void Init(int n,T num=0){mat_siz=n;mat.clear();vector<T>v;for(int i=0;i<=n;i++)v.push_back(num);for(int i=0;i<=n;i++)mat.push_back(v);}
	il void Init_01(int n){Init(n);for(int i=1;i<=n;i++)mat[i][i]=1;}};
	tpcTi matrix<T>mat_mul(cst matrix<T>&x,cst matrix<T>&y,cst ll&p){matrix<T>ans;ans.Init(x.mat_siz);int n=x.mat_siz;for(int i=1;i<=n;i++)for(int j=1;j<=n;j++)for(int k=1;k<=n;k++)ans.mat[i][j]=madd(ans.mat[i][j],x.mat[i][k]*y.mat[k][j]%p,p);return ans;}
	...
	void NTT(int*a,int len,bool INTT){int*r=new int[len];r[0]=0;int l=-1;for(int x=len;x;x>>=1)++l;for(int i=1;i<len;i++)r[i]=(r[i/2]/2)|((i&1)<<(l-1));for(int i=0;i<len;i++)if(i<r[i])_swap<int>(a[i],a[r[i]]);delete[]r;for(int i=1;i<len;i<<=1){int p=(i<<1);int wn=pw(3,(Mod-1)/p,Mod);if(INTT)wn=pinv(wn,Mod);for(int j=0;j<len;j+=p){int w=1;for(int k=0;k<i;k++){int x=a[j+k],y=1ll*w*a[i+j+k]%Mod;a[j+k]=(x+y)%Mod;a[i+j+k]=(x-y+Mod)%Mod;w=1ll*w*wn%Mod;}}}if(INTT){int inv=pinv(len,Mod);for(int i=0;i<len;i++)a[i]=1ll*a[i]*inv%Mod;}}
	class Polynomial{
		public:
			int n;
		    int *a;
	...
	Polynomial Polynomial::operator+(cst Polynomial&rhs)cst{Polynomial res(std::max(n,rhs.n));for(int i=0;i<=n;i++)res[i]=(*this)[i];for(int i=0;i<=rhs.n;i++)res[i]=(res[i]+rhs[i])%Mod;ret res;}
	Polynomial Polynomial::operator-(cst Polynomial&rhs)cst{Polynomial res(std::max(n,rhs.n));for(int i=0;i<=n;i++)res[i]=(*this)[i];for(int i=0;i<=rhs.n;i++)res[i]=(res[i]-rhs[i]+Mod)%Mod;ret res;}
...
il ll S(int l,int r){ret S(r)-S(l-1);}
int main(void){
